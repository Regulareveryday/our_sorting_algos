...
idempotent_bubblesort xs 	= bubblesortsort (bubblesortsort xs) == qbubblesortsort xs
idempotent_insertionsort xs = insertionsort (insertionsort xs) 	 == insertionsort xs
idempotent_mergesort xs 	= mergesort (qmergesort xs) 		 == mergesort xs
...
compare_elements_quicksort xs = ordered_elements (quicksort xs)
    	where ordered_elements []       = True
          	  ordered_elements [x]      = True
          	  ordered_elements (x:y:xs) = x <= y && ordered (y:xs)
